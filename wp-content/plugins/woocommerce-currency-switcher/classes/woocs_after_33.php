...
//+++
        ...
        if (!$is_first_activation) {
            ...
            update_option('woocs_activate_page_list_reverse', 1);
...
//***
        ...
        if (is_admin()) {
            $this->rate_alert = new WOOCS_RATE_ALERT($this->notes_for_free);
        }
    }
...
//*** checking and assigning data
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['tab']) AND $_GET['tab'] == 'woocs') {
            wp_enqueue_style('woocs-options', WOOCS_LINK . 'css/options.css', array(), WOOCS_VERSION);
...
//[woocs_show_custom_price value=20] -> value should be in default currency
    public function woocs_show_custom_price($atts) {
        ...
        foreach ($currencies as $currency) {
            $cache_data[$currency['name']] = $this->wc_price($value, $convert, array('currency' => $currency['name']));
        }
        unset($_REQUEST['woocs_show_custom_price']);
        return '<span class="woocs_amount_custom_price" data-value="' . $value . '">' . $wc_price . "</span>";
    }
...
//http://stackoverflow.com/questions/6918623/curlopt-followlocation-cannot-be-activated
    ...
    public function get_rate() {
        ...
        $custom_rate = apply_filters("woocs_add_custom_rate", FALSE, $this->default_currency, $_REQUEST['currency_name']);
...
//***
            ...
            case 'micro':
                ...
                $ex_currency = $this->escape($_REQUEST['currency_name']);
                $query_url = 'https://api.ratesapi.io/api/latest?base=' . $this->default_currency . '&symbols=' . $currency_name;
                if (function_exists('curl_init') AND $woocs_use_curl) {
                    $res = $this->file_get_contents_curl($query_url);
                } else {
                    $res = file_get_contents($query_url);
...
//+++
        if (!empty($customer_price_format)) {
            ...
            $txt = '<span class="' . $classes . '" data-product-id="' . $product_id . '">' . $customer_price_format . '</span>';
