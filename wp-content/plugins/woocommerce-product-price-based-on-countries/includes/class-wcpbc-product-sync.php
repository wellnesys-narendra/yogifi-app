...
class WCPBC_Product_Sync {
		...
		foreach ( WCPBC_Pricing_Zones::get_zones() as $zone ) {
			...
			foreach ( $product_ids as $product_id ) {
				if ( $zone->is_exchange_rate_price( $product_id ) ) {
					$price = $zone->get_exchange_rate_price_by_post( $product_id, '_price', false );
	...
	public static function update_exchange_rate_prices( $zone_id, $metakey ) {
		...
		$updated          = 0;
		$querys           = array();
		$s_exchange_rate  = "CASE TRIM(COALESCE(meta__price.meta_value, '')) WHEN '' THEN '' ELSE ROUND((meta__price.meta_value+0) * (%s), 8) END";
