...
class WCPBC_Pricing_Zone {
	 ...
	 * @return mixed
	 */
	public function get_postmeta( $post_id, $meta_key, $single = true ) {
		return get_post_meta( $post_id, $this->get_postmetakey( $meta_key ), $single );
	}
	 ...
	 * @return float
	 */
	public function get_exchange_rate_price_by_post( $post_id, $meta_key, $round = false ) {
	...
	public function get_price_prop( $data, $value, $meta_key, $context = 'product' ) {
		...
		if ( $this->is_exchange_rate_price( $data ) ) {
			...
			if ( floatval( $price ) !== floatval( $_price ) ) {
				$price = $_price;
				$this->set_postmeta( $data->get_id(), $meta_key, $price );
			}
